...
def process(subject_name, bands, selected_channels, reg=None, balanced=True, classifier='LDA'):
    runtime = []
    tmin, tmax = 0., 2.
...
def main(subjects_id):
    ...
    confusion_matrices = []
    times = {'patient': [], 'classifier': [], 'train': [], 'test': []}
    ...
    for patient_index in subjects_id:
        ...
        times['patient'].append(patient_index)
        times['classifier'].append('MLP')
        times['train'].append(runtime[0])
        times['test'].append(runtime[1])

    times = pd.DataFrame(data=times)
    times.to_csv('times_classifier.csv')
