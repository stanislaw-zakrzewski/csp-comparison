...
def main(subjects_id):
    ...
    confusion_matrices = []
    all_scores = {'patient': [], 'accuracy': [], 'structure': []}
    individual_classes = {'patient': [], 'accuracy': [], 'class': [], 'structure': []}
    ...
    for patient_index in subjects_id:
        ...
        for i in range(len(predictions)):
            ...
            all_scores['structure'].append('multiclass balanced')
            for indx, elem in enumerate(get_individual_accuracy(predictions[i], corrects[i])):
                if elem <= 1:
                    ...
                    individual_classes['structure'].append('multiclass balanced')
        ...
        for i in range(len(predictions)):
            all_scores['patient'].append(patient_index)
            all_scores['accuracy'].append(accuracy_score(predictions[i], corrects[i]))
            all_scores['structure'].append('multiclass unbalanced')
            for indx, elem in enumerate(get_individual_accuracy(predictions[i], corrects[i])):
                if elem <= 1:
                    individual_classes['patient'].append(patient_index)
                    individual_classes['class'].append(indx)
                    individual_classes['accuracy'].append(elem)
                    individual_classes['structure'].append('multiclass unbalanced')
        chances['patients'].append(patient_index)
        chances['values'].append(st.binom.ppf(.9, len(predictions[0]), .3) / len(predictions[0]))

        window_times, window_scores, csp_filters, epochs_info, predictions, corrects = process_hierarchical(
            patient_name,
            chosen_bands, channels)
        for i in range(len(predictions)):
            all_scores['patient'].append(patient_index)
            all_scores['accuracy'].append(accuracy_score(predictions[i], corrects[i]))
            all_scores['structure'].append('hierarchical')
            for indx, elem in enumerate(get_individual_accuracy(predictions[i], corrects[i])):
                if elem <= 1:
                    individual_classes['patient'].append(patient_index)
                    individual_classes['class'].append(indx)
                    individual_classes['accuracy'].append(elem)
                    individual_classes['structure'].append('hierarchical')
        chances['patients'].append(patient_index)
        chances['values'].append(st.binom.ppf(.9, len(predictions[0]), .3) / len(predictions[0]))
    ...
    individual_classes.to_csv('individual_classes.csv')
    chances = pd.DataFrame(chances['values'], index=chances['patients']).transpose()

    _accuracy_fig, accuracy_ax = plt.subplots()
    accuracy_plot = sns.barplot(x="patient", y="accuracy", hue='structure', data=all_scores, ci="sd", ax=accuracy_ax,
                                zorder=0)
